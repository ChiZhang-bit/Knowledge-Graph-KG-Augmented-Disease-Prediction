...
def eval(model, data_eval, voc_size, epoch):
    ...
    '''
    llprint(
        '\tDDI Rate: %.4f , Jaccard: %.4f $ %.4f $,  PRAUC: %.4f $ %.4f $, AVG_PRC: %.4f $ %.4f $, AVG_RECALL: %.4f $ %.4f $, AVG_F1: %.4f $ %.4f $\n' % (
            ddi_rate, np.mean(ja), np.std(ja), np.mean(prauc), np.std(prauc), np.mean(avg_p), np.std(avg_p),
            np.mean(avg_r), np.std(avg_r), np.mean(avg_f1), np.std(avg_f1)
        ))
